...
const ManageEmbellishmentScreen: React.FC = () => {
    const { sales } = useContext(AppContext);
    const [selectedSale, setSelectedSale] = useState<Sale | null>(null);

    const relevantSales = useMemo(() => {
        return sales.filter(s => s.hasEmbellishment);
    }, [sales]);

    return (
        <div>
            {selectedSale && <SaleDetailModal sale={selectedSale} onClose={() => setSelectedSale(null)} />}
            <h2 className="text-2xl font-bold text-gray-800 mb-6">Gerenciar Embelezamento</h2>
            
            <p className="text-gray-600 mb-6">Abaixo estão as vendas que requerem serviços de embelezamento. Clique em "Gerenciar Preparo" para atualizar os detalhes.</p>
            ...
            <div className="space-y-4">
                {relevantSales.length > 0 ? (
                    ...
                    <div className="text-center py-10 bg-white rounded-lg shadow-sm">
                        <p className="text-gray-600">Nenhuma venda com embelezamento encontrada.</p>
                    </div>
                )}
            </div>
        </div>
...
export default ManageEmbellishmentScreen;
